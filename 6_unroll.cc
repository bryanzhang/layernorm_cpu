...
void layernorm_forward_cpu(float* out, float* mean, float* rstd,
    ...
    for (int b = 0; b < B; b++) {
        for (int t = 0; t < T; t++) {
            ...
            // calculate the mean
	    __m512 v_sum1 = _mm512_setzero_ps();
	    __m512 v_sum2 = _mm512_setzero_ps();
	    __m512 v_sqr1 = _mm512_setzero_ps();
	    __m512 v_sqr2 = _mm512_setzero_ps();
	    for (int i = 0; i < C; i += 32) {
		__m512 va = _mm512_loadu_ps(x + i);
		v_sum1 = _mm512_add_ps(v_sum1, va);
		v_sqr1 = _mm512_fmadd_ps(va, va, v_sqr1);
		__m512 vb = _mm512_loadu_ps(x + i + 16);
		v_sum2 = _mm512_add_ps(v_sum2, vb);
		v_sqr2 = _mm512_fmadd_ps(vb, vb, v_sqr2);
	    }
	    v_sum1 = _mm512_add_ps(v_sum1, v_sum2);
	    v_sqr1 = _mm512_add_ps(v_sqr1, v_sqr2);
	    float m = _mm512_reduce_add_ps(v_sum1) / C;
	    float m_sqr = _mm512_reduce_add_ps(v_sqr1) / C;
